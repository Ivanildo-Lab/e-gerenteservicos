...
def dashboard(request):
    ...
    # 2. DADOS FINANCEIROS MENSAL (KPIs 2, 3 e 4)
    # Receitas Realizadas (Créditos no Caixa)
    ...
    # Despesas Realizadas (Débitos no Caixa)
    ...
    # 3. LISTAS DE ALERTAS (ATRASADOS)
    
    # Recebimentos em Atraso (Contas do tipo Receita 'R')
    contas_atrasadas = Conta.objects.filter(
        empresa=empresa,
        plano_de_contas__tipo='R',
        status='PENDENTE',
        data_vencimento__lt=hoje
    ).order_by('data_vencimento')[:10]

    # Pagamentos em Atraso (Contas do tipo Despesa 'D') - NOVO
    pagamentos_atrasados = Conta.objects.filter(
        empresa=empresa,
        plano_de_contas__tipo='D',
        status='PENDENTE',
        data_vencimento__lt=hoje
    ).order_by('data_vencimento')[:10]
    ...
    for i in range(5, -1, -1):
        # Calcula o primeiro dia de cada um dos últimos 6 meses
    ...
    context = {
        ...
        'pagamentos_atrasados': pagamentos_atrasados, # Variável nova enviada ao template
